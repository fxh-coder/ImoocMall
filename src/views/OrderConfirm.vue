        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item, index) in cartListCom" :key="index">
                <!-- 商品图片和商品名称 -->
  ...
  methods: {
    init() {
      ...
      axios.get('/users/cartList').then(response => {
        // 订单确认列表不需要再写接口，直接用购物车列表的接口，渲染页面时选取选中的商品作为订单确认的商品
        ...
        this.orderTotal =
          this.subTotal + this.shipping - this.discount + this.tax // 获取用户最终支付的金额，(总金额+配送费-折扣+税费)
      })
    },
    payMent() {
      // 点击支付
